...
const FileStoreService = {
    ...
    async writeChatMessage(channel: string, user: string, text: string, msg: VoddingChatMessage): Promise<void> {
        const db = await JSONFilePreset<ChatMessageData>(filePath(channel), defaultData)
    ...
    async getChatMessages(channel: string, author?: string): Promise<datastoreChatMesssage[]> {
        const db = await JSONFilePreset<ChatMessageData>(filePath(channel), defaultData);
        ...
        if (author) {
            messages = messages.filter(message => message.user === author.toLowerCase());
        }
        return messages;
    },

    async getChatMessagesBetweenDates(channel: string, startDate: Date, endDate: Date, author?: string): Promise<datastoreChatMesssage[]> {
        return await FileStoreService.getChatMessages(channel, author).then((messages) => {
